...
class Box:
    ...
    def draw_on(self, canvas, cbbcs, pbbcs, color='black'):
        ...
        w.create_Box(x,y,X,Y, outline=color)
    def center(self):
    ...
    def join_with(self, other):
        self.coors = [[(min,max)[i](self.coors[i][j]) for j in range(2)] for i in range(2)]
    ...
    def random_pt_within(self):
       return Box([random()*(coors[1][i]-coors[0][i])+coors[1][i] for i in range(2)])
